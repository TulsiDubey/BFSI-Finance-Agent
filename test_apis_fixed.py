#!/usr/bin/env python3
"""
Fixed API Testing Script for BFSI Finance Agent
...
def test_endpoint(endpoint, method="GET", data=None, expected_status=200, files=None):
...
def test_all_apis():
    """Test all API endpoints with proper data formats"""
    ...
    # Test 2: Root Endpoint (should work now)
    ...
    # Test 8: Security Analysis API (with file upload)
    ...
    result = test_endpoint("/api/security/analyze", method="POST", data=data, files=files)
    test_results.append(result)
    print(f"   Status: {result['status_code']} | Success: {result['success']}")
    ...
    # Test 10: Add Policy API (with required 'term' field)
    ...
    # Summary Report
    print("\n" + "=" * 60)
    print("📊 FIXED API TESTING SUMMARY")
    print("=" * 60)
    ...
    for i, result in enumerate(test_results, 1):
        ...
        elif not result.get('success', False) and 'response_data' in result:
            print(f"    Response: {result['response_data']}")
